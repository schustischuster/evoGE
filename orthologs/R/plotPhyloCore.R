...
#---------------------- Plot pairwise orthologs for DevSeq marker species ----------------------
   ...
   brass_orthologs <- as.numeric(pairwise_ortholog[8,2:3])
     ...
     plotOrthologs <- function(data) {
        ...
        dat_circle_brass <- data.frame(
               class   = c("coding", "lncRNA"),
               x     = c(26, 26),
               y     = c(brass_orthologs[1], brass_orthologs[2])
           )
        ...
        dat_text_brass <- data.frame(
               label = c(brass_orthologs[1], brass_orthologs[2]),
               class   = c("coding", "lncRNA"),
               x     = c(26, 26),
               y     = c(brass_orthologs[1] - 2450, brass_orthologs[2] + 175)
           )
        ...
        geom_point(data = dat_circle_brass, mapping = aes(x = x, y = y), shape = 21, colour = "black", 
          fill = "green4", size = 3.0, stroke = 0.75) + 
        geom_text(data = dat_text_brass, mapping = aes(x = x, y = y, label=label), color = "green4", size = 3.8)
