...
plotGOs <- function(...) {
    ...
    plotGOslimStsM <- function(df) {
        ...
        p <- ggplot(df, aes(x = p_value_FDR, y = goslim_term, colour = color)) +
        geom_point(mapping=aes(size = ortho_genes, colour = color)) + 
        scale_x_continuous(expand = c(0.05, 0), limits = c(2, 5), breaks = c(2, 2.5, 3, 3.5, 4, 4.5, 5), 
            labels = c("2", "", "3", "", "4", "", "5")) + 
        scale_y_discrete(expand = c(0.075, 0)) + 
        scale_color_identity(breaks = c("#f20000", "#5972b5"), labels = c("High", "Low"), 
            guide = "legend", name = "Divergence") + 
        scale_size_continuous(range = c(5.9, 10.25), breaks = c(415, 600, 800, 1100, 1600), 
            labels = c("415", "600", "800", "1100", "1600"), name = "Gene count") + 
        guides(size = guide_legend(order = 2), colour = guide_legend(order = 1, override.aes=list(size = 8))) + 
        labs(x = expression(-log[10]*"(FDR)"), y = NULL) + 
        ggtitle("Gene expression divergence \nacross functional groups") + 
        theme(panel.background = element_blank(), 
            axis.ticks.length = unit(0.285, "cm"), 
            axis.ticks = element_line(colour = "black", size = 1.1), 
            axis.line = element_line(colour = 'black', size = 1.1), 
            plot.margin = unit(c(1.1, 2.25, 0, 2.75), "cm"), 
            plot.title = element_text(size=21.5, margin = margin(t = 8, r = 0, b = 10, l = 0), hjust = 0.5),
            legend.position = c(0.82, 0.41), 
            legend.text = element_text(size=18.5), 
            legend.title = element_text(size=19.0),
            legend.direction = "vertical", 
            legend.box = "vertical",
            legend.key = element_blank(),
            legend.margin=margin(t = 0.228, l = 0.41, b = 0, unit='cm'),
            axis.title.y = element_text(size=21.5, margin = margin(t = 0, r = 7.0, b = 0, l = 10), 
                colour="black", face = "bold"), 
            axis.title.x = element_text(size=21.5, margin = margin(t = 0, r = 0, b = 1, l = 0), 
                colour="black", face = "bold"), 
            axis.text.x = element_text(size=19, margin = margin(t = 4.25, b = 7.15), colour="grey20"), 
            axis.text.y = element_text(size=19.75, angle=0, margin = margin(l = 8.8, r = 4), colour="grey20")
        )
